...
export default function Card({ type, title, description, imageSrc, buttonText, link }: CardProps) {
  ...
  return (
    <div className={`p-4 rounded-lg shadow-xl bg-secondary ${gridClasses} hover:shadow-xl hover:-translate-y-2 transition-transform duration-300 cursor-pointer flex flex-col justify-between`}>
      <Link href={link}>
        <div className="h-52 w-full relative rounded-lg overflow-hidden">
          {imageSrc ? (
            ...
            <div className="flex flex-col items-center justify-center w-full h-full bg-stroke rounded-lg gap-y-2">
        ...
        <p className="mb-5 text-base lg:text-lg">{description}</p>
      </Link>
      <div className="w-full flex justify-end">
        <Link href={link}>
          <button className="px-6 py-2 text-base font-semibold transition duration-300 ease-in-out text-white bg-primary rounded-md hover:shadow-md hover:shadow-primary w-fit">{buttonText}</button>
        </Link>
      </div>
    </div>
  );
